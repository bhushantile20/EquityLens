    ...
    >
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-center gap-3">
          <div
            className="w-10 h-10 rounded-xl bg-base border border-border flex items-center justify-center
          ...
          <div>
            ...
            {portfolio.description && (
              <p className="text-xs text-slate-600 mt-0.5 truncate max-w-[160px]">
                {portfolio.description}
              </p>
            )}
          </div>
        ...
        <span
          className={`text-xs font-display px-2 py-1 rounded ${plPct >= 0 ? "text-profit bg-profit/10" : "text-loss bg-loss/10"}`}
        >
          {plPct >= 0 ? "+" : ""}
          {plPct.toFixed(2)}%
        </span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-sm font-display text-slate-300">
            ${invested.toLocaleString()}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-600 font-display mb-0.5">
            Current Value
          </p>
          <p className="text-sm font-display text-slate-300">
        ...
        <div>
          ...
          <p className="text-sm font-display text-slate-300">
            {portfolio.holdings_count ?? 0} stocks
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-600 font-display mb-0.5">P/L</p>
          <p
            className={`text-sm font-display ${pl >= 0 ? "text-profit" : "text-loss"}`}
          >
            {pl >= 0 ? "+" : ""}$
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-24 border border-border rounded-2xl bg-surface/20">
          ...
          <h2 className="text-slate-300 font-semibold mb-2">
            No portfolios yet
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            <div
              className="w-10 h-10 rounded-xl border border-dashed border-border group-hover:border-accent/40
                            flex items-center justify-center text-slate-600 group-hover:text-accent transition-colors text-xl"
            >
              +
            </div>
            <p className="text-sm text-slate-600 group-hover:text-slate-400 transition-colors">
              Add Portfolio
            </p>
          </div>
        </div>
