...
function Stat({ label, value, color, sub }) {
  return (
    <div className="stat-card">
      <p className="text-xs text-slate-500 font-display uppercase tracking-wider mb-1">{label}</p>
      <p className={`text-xl font-display font-semibold ${color ?? "text-slate-100"}`}>{value ?? "—"}</p>
...
function RsiBadge({ value }) {
  ...
  let label = "Neutral";
  if (n > 70) { color = "text-loss bg-loss/10"; label = "Overbought"; }
  if (n < 30) { color = "text-profit bg-profit/10"; label = "Oversold"; }
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-display ${color}`}>
...
export default function StockAnalytics() {
  ...
  const currPrice = parseFloat(analysis?.current_price ?? holding?.current_price ?? buyPrice);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-end gap-4 justify-between mb-8">
        ...
        <div className="text-right">
          ...
          {analysis?.change_pct != null && (
            <p className={`text-sm font-display ${parseFloat(analysis.change_pct) >= 0 ? "text-profit" : "text-loss"}`}>
              {parseFloat(analysis.change_pct) >= 0 ? "+" : ""}{parseFloat(analysis.change_pct).toFixed(2)}% today
            </p>
          )}
        </div>
      </div>
      ...
      {holding && (
        <>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-slate-600 font-display uppercase tracking-wider px-2">Your Position</span>
            <div className="h-px flex-1 bg-border" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            <Stat label="Quantity" value={qty.toLocaleString()} sub="shares held" />
            <Stat label="Buy Price" value={`$${buyPrice.toFixed(2)}`} sub="avg. cost" />
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="h-px flex-1 bg-border" />
        <span className="text-xs text-slate-600 font-display uppercase tracking-wider px-2">Market Analytics</span>
        <div className="h-px flex-1 bg-border" />
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <Stat label="P/E Ratio"
          value={analysis?.pe_ratio != null ? parseFloat(analysis.pe_ratio).toFixed(2) : null} />
        <Stat label="RSI"
          value={analysis?.rsi != null ? parseFloat(analysis.rsi).toFixed(1) : null} />
        <Stat label="Volatility"
          value={analysis?.volatility != null ? `${parseFloat(analysis.volatility).toFixed(2)}%` : null}
          color={parseFloat(analysis?.volatility) > 30 ? "text-loss" : "text-profit"} />
        <Stat label="Opp. Score"
          value={analysis?.opportunity_score != null ? parseFloat(analysis.opportunity_score).toFixed(0) : null}
          color={parseFloat(analysis?.opportunity_score) >= 70 ? "text-profit" : parseFloat(analysis?.opportunity_score) < 30 ? "text-loss" : "text-accent-light"} />
      </div>

      {/* Chart */}
      <div className="bg-surface border border-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-display text-slate-400 uppercase tracking-wider">Price History</h2>
