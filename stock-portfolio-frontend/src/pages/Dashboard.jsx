...
function WelcomeBanner() {
  ...
  return (
    <div className="relative rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/10 to-transparent overflow-hidden p-8 mb-8">
      ...
      <p className="relative text-xs font-display text-accent uppercase tracking-widest mb-2">
        Dashboard
      </p>
      <h1 className="relative text-2xl font-bold text-white">{greeting} 👋</h1>
      <p className="relative text-slate-400 text-sm mt-1">
...
function SummaryCard({ label, value, color, icon }) {
  return (
    <div className="stat-card flex items-center gap-4">
      ...
      <div>
        <p className="text-xs text-slate-500 font-display uppercase tracking-wider">
          {label}
        </p>
        <p
          className={`text-xl font-display font-semibold mt-0.5 ${color ?? "text-slate-100"}`}
        >
          {value}
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-right">
        ...
        <p
          className={`text-xs font-display ${pl >= 0 ? "text-profit" : "text-loss"}`}
        >
          {pl >= 0 ? "+" : ""}
          {pl.toFixed(2)}%
        </p>
...
export default function Dashboard() {
  ...
  const totalInvested = portfolios.reduce(
    (s, p) => s + parseFloat(p.total_invested ?? 0),
    0,
  );
