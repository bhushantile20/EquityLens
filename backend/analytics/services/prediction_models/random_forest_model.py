...
def predict(ticker, days_ahead=7, period="1y", interval="1d"):
    """
    True Random Forest with lagged features (proper implementation,
    not shared with CNN).  Uses 5-day lag window + rolling mean/std features.
    """
    ...
    val_model = RandomForestRegressor(n_estimators=200, random_state=42, n_jobs=-1)
